            ...
            if (!$validator->fails()) {
                ...
                if ($execQuery->first()) {
                    ...
                    foreach ($execQuery as $execQuery_row) {
                        ...
                        $responseData->image = isset($execQuery_row->image) && $execQuery_row->image ? URL::to("/upload/team/" . $execQuery_row['id'] . "/" . $execQuery_row['image']) : NULL;
                        $responseData->image = isset($execQuery_row->image) && $execQuery_row->image ? URL::to("/upload/team/" . $execQuery_row['id'] . "/" . $execQuery_row['image']) : NULL;
        ...
        try {
            ...
            if (!$validator->fails()) {
                ...
                if ($getInfoTeam) {
                    ...
                    $responseData->image = isset($getInfoTeam->image) && $getInfoTeam->image ? URL::to("/upload/team/" . $getInfoTeam['id'] . "/" . $getInfoTeam['image']) : NULL;
                    $responseData->personnel = array();
                            ...
                            foreach ($checkTeamTournament as $teamTournamentRow) {
                                ...
                                if ($tournamentInfo) {
                                    $data = array(
                                        ...
                                        'tournament_image' => isset($tournamentInfo->image) && $tournamentInfo->image ? URL::to("/upload/tournament/" . $tournamentInfo->id . "/" . $tournamentInfo->image) : '',
