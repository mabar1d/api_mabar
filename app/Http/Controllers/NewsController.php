...
class NewsController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        try {
            ...
            if (!$validator->fails()) {
                ...
                $resultData = array();
                $getList = NewsModel::select('*')
                    ->where("status", $status)
                    ->offset($offset)
                    ->limit($limit);
                if ($search) {
                    $getList->where('title', 'like', $search . '%');
                }
                $getList->orderBy($orderBy, $orderByMethod);
                if ($getList->count() == 0) {
                ...
                foreach ($getList as $row) {
                    ...
                    $getInfo = [
                        ...
                        "content" => $row->content,
                        "image" => $row->image,
                        "news_image_url" => URL::to("/upload/news/" . $row["image"]),
                        "tag" => $newsTags,
                        "status" => $row->status,
        ...
        try {
            ...
            if (!$validator->fails()) {
                ...
                $getInfo = [
                    ...
                    "image" => $data->image,
                    "news_image_url" => URL::to("/upload/news/" . $data["image"]),
                    "tag" => $newsTags,
                    "status" => $data->status,
    ...
    public function count(Request $request)
    {
        ...
        try {
            ...
            $status = isset($requestData['status']) && $requestData['status'] ? trim($requestData['status']) : 1;
            if (!$validator->fails()) {
                $query = NewsModel::where("status", $status);
                $execQuery = $query->count();

                $response->code = '00';
                $response->desc = 'Count News Success!';
                $response->data = [
